...
const getBookingDetails = catchAsync(async (req, res) => {
    const result = await BookingServices.getBookingDetails(req.params.id, req.user.id);
    sendResponse(res, { statusCode: 200, success: true, message: "Booking details retrieved successfully", data: result });
});
const cancelBooking = catchAsync(async (req, res) => {
    const result = await BookingServices.cancelBooking(req.params.id, req.user.id);
