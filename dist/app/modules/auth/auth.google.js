...
export function signGoogleOAuthState(payload) {
    return jwt.sign({ purpose: 'google_oauth', role: payload.role, next: payload.next }, config.jwt_access_secret, { expiresIn: '10m' });
}
export function readGoogleOAuthState(state) {
    const decoded = jwt.verify(state, config.jwt_access_secret);
