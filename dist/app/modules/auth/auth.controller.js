...
import { buildGoogleAuthUrl, exchangeGoogleCode, isGoogleOAuthConfigured, readGoogleOAuthState, signGoogleOAuthState, } from './auth.google.js';
function requestOrigin(req) {
    const proto = (req.get('x-forwarded-proto') ?? req.protocol ?? 'http').split(',')[0];
    const host = req.get('host') ?? 'localhost:5000';
    return `${proto}://${host}`.replace(/\/$/, '');
}
function oauthFrontendBase(req) {
...
function frontendAuthError(req, code) {
    return `${oauthFrontendBase(req)}/login?error=${encodeURIComponent(code)}`;
}
function frontendAuthCallback(req, accessToken) {
...
const demoLoginUser = catchAsync(async (req, res) => {
...
const googleStart = catchAsync(async (req, res) => {
