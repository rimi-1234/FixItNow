...
function frontendBase(req) {
    ...
    if (configured && sameHost(configured, origin))
        return KNOWN_FRONTEND_FALLBACK;
    return configured || KNOWN_FRONTEND_FALLBACK;
}
function shouldRedirectToFrontend(req, front) {
    if (!front)
        return false;
    ...
    const origin = requestOrigin(req);
    if (sameHost(front, origin))
        return false;
    const appUrl = (config.app_url || "").replace(/\/$/, "");
    if (appUrl && front !== appUrl)
        return true;
    const host = req.get("host") ?? "";
    if (front.includes("localhost:3000") && !host.includes("3000"))
        return true;
    return false;
}
...
app.get("/payment/success", async (req, res) => {
    ...
    res.status(200).send(paymentResultHtml({
        variant: "success",
