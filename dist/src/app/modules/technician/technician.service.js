...
const withAverageRating = (technician) => {
...
const getTechnicianById = async (id) => {
    ...
    if (!technician)
        throw Object.assign(new Error('Technician not found'), { statusCode: 404 });
    const { reviewsReceived, ...rest } = technician;
    const reviewCount = reviewsReceived.length;
    const averageRating = reviewCount
        ? Number((reviewsReceived.reduce((sum, r) => sum + r.rating, 0) / reviewCount).toFixed(2))
...
const updateAvailability = async (technicianId, availability) => {
...
const updateBookingStatus = async (technicianId, bookingId, status) => {
